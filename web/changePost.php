    ...
    <div class="post-info">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="input-group1">
                <button class="btn1" name="del" value="delete" type="submit">Удалить</div>
        </form>.
    </div>
</div>
...
if ($tmp!=$post_info['img_post']){
   $result = mysqli_query($conn, "UPDATE posts SET `image`='$tmp' WHERE `idPost`=".$post_info[
...
$delit = $_POST['del'];
 if ($delit == "delete"){
   $result = mysqli_query($conn, "DELETE FROM posts WHERE `idPost`=".$post_info['idPost']);
    if ($result){
        unset( $_POST['del']);
    }
 }
